...
export class NavbarComponent  implements OnInit {
  ...
  constructor(private sessionStorage: SessionStorageService, private router: Router, private cartService:CartService) {}

  ngOnInit(): void {
    const token = this.sessionStorage.getItem('token');
    this.isAdmin = token && token.type === 'ADMIN';
    this.isLoggedIn = !!token;
    this.cartItemsQty = this.cartService.totalCart();
  }
  handleUserIconClick() {
    if (this.isLoggedIn) {
